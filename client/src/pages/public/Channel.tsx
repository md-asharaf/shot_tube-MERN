...
const Channel = () => {
  ...
  const { mutate: toggleSubscription } = useMutation({
    mutationFn: async () => {
      await subscriptionServices.toggleSubscription(userDetails?.user?._id);
    },
    onSuccess: () => {
      refetch();
      return true;
    ...
    <div className="space-y-4 w-full">
      ...
      <div className="flex space-x-6 justify-center">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-bold">{userDetails?.user?.fullname}</h2>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {userVideos?.map((video, index) => (
          ...
          >
            <VideoCard thumbnail={video.thumbnail} duration={video.duration} />
            <VideoTitle
              video={{
                _id: video._id,
                views: video.views,
